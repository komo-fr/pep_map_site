...
from .style_setting import STATUS_COLOR_MAP_DICT
...
def generate_timeline_data_source(source_graph: Graph) -> ColumnDataSource:
    ...
    df['PythonVersion'] = df['Python-Version'].apply(
        lambda x: x if x == x else 'No Data')

    df = df[['PEP', 'Title', 'Type', 'Status',
             'Created', 'Created_dt', 'status_node_color', 'y', 'PythonVersion']]
...
def setup_timeline_axis(plot: figure) -> None:
    ...
    dt_dick = list(pd.date_range(start=start_date,
                                 ...
                                 freq='6M'))
    dt_dick_values = list(pd.to_datetime(dt_dick).astype(int) / 10 ** 6)

    # 補助目盛り幅の作成
    minor_dt_dick = list(pd.date_range(start=start_date,
                                       end=end_date,
                                       freq='6M'))
    minor_dt_dick_values = list(pd.to_datetime(minor_dt_dick).astype(int) / 10 ** 6)

    # 軸の設定
    plot.xaxis.axis_label = "Creation date of the PEPs (Some PEPs don't have 'Created' field)"
...
def setup_timeline_tools(plot: figure) -> None:
    ...
    release_hover = HoverTool(tooltips=[('Version', '@release_number'),
                          ...
                          names=["release_label"])
    plot.add_tools(timeline_hover, release_hover, BoxZoomTool(), ResetTool(), SaveTool())
...
def generate_release_label_source(source_df: pd.DataFrame,
    ...
    df = source_df.copy()
    PYTHON_YELLOW_COLOR_CODE = '#DDAD3E'
    PYTHON_BLUE_COLOR_CODE = '#2E6495'
